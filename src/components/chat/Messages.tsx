        ...
        return (
          ...
          >
            <div className="w-[90%] max-w-3xl py-6 mx-auto flex sm:gap-6 gap-3">
              {isChatGPT ? (
                <div className="bg-[#19C37D] min-w-[30px] max-w-[30px] h-[30px] flex items-center justify-center rounded-sm">
                  <GptIcon />
                </div>
              ) : (
                <div className="bg-[#EC427B] min-w-[30px] max-w-[30px] h-[30px] flex items-center justify-center rounded-sm">
              ...
              <div className="min-[1115px]:max-w-full max-w-[85%]">
                {messageParts.map((part, index) => {
                  if (part.startsWith("```")) {
                    ...
                    const language = codeBlockParts[0]
                      .replace(/```/, "")
                      .trim();
                    ...
                    return (
                      <div key={index}>
                        {paragraphs.map((paragraph, pIndex) => (
                          <p
                            className={`text-sm leading-normal sm:leading-[1.7rem] sm:text-base ${
                              isChatGPT
                                ? "text-[#D1D5DB] mt-[20px] first:mt-0"
                                : "text-[#ECECF1]"
                            }`}
                            key={pIndex}
                          >
